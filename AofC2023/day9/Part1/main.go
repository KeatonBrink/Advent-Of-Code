...
func main() {
	...
	for _, line := range input_history {
		...
		for depth := len(individual_line_decomposition) - 1; depth > 0; depth-- {
			...
			new_val = higher_line[len(higher_line)-1] + lower_line[len(lower_line)-1]
			individual_line_decomposition[depth-1] = append(individual_line_decomposition[depth-1], new_val)
		}
		// println(new_val)
		score += new_val
	}
