...
func main() {
	...
	var all_success_paths []TrailPath

	for success_path := range success_chan {
		already_success := false
		for _, confirmed_path := range all_success_paths {
			if confirmed_path.head_x == success_path.head_x && confirmed_path.head_y == success_path.head_y && confirmed_path.peak_x == success_path.peak_x && confirmed_path.peak_y == success_path.peak_y {
				already_success = true
				break
			}
		}
		if !already_success {
			all_success_paths = append(all_success_paths, success_path)
		}
	}

	fmt.Println("Final: ", len(all_success_paths))
}
